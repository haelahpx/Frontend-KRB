        ...
        <div class="{{ $card }}">
            ...
            <div class="p-4 grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
                <div class="md:col-span-2">
                    ...
                    <input type="text" wire:model.debounce.500ms="search" class="{{ $input }}" placeholder="Name / plate / year">
                </div>
                <div>
            ...
            <div class="{{ $card }}">
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        {{-- hidden fields (IMPORTANT: wire:model on JSON) --}}
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        @foreach($rows as $r)
                            <tr wire:key="row-{{ $r->vehicle_id }}">
                                <td class="py-2">
                                    @if($r->image)
                                        <img src="{{ $r->image }}" class="h-12 w-16 object-cover rounded-lg border" alt="Vehicle">
                                    @else
                                        <div class="h-12 w-16 bg-gray-200 rounded-lg grid place-items-center text-xs text-gray-500">No Image</div>
                                    @endif
                                </td>
                                ...
                                <td class="py-2">
                                    <span class="px-2 py-0.5 rounded text-xs {{ $r->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600' }}">
                                        {{ $r->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="py-2 text-right space-x-2">
                                    @if(!$showTrashed)
                                        <button class="{{ $btnLite }}" wire:click="openEdit({{ $r->vehicle_id }})">Edit</button>
                                        <button class="{{ $btnRed }}" wire:click="delete({{ $r->vehicle_id }})">Trash</button>
                                    @else
                                        <button class="{{ $btnLite }}" wire:click="restore({{ $r->vehicle_id }})">Restore</button>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
                    ...
                    <div class="px-6 py-5 space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                @error('edit_plate_number') <p class="text-rose-600 text-sm mt-1">{{ $message }}</p> @enderror
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="pt-7">
                                <label class="inline-flex items-center gap-2">
                                    <input type="checkbox" wire:model.defer="edit_is_active" class="rounded border-gray-300">
                                    <span class="text-sm text-gray-700">Active</span>
                                </label>
                            </div>
                        </div>
    ...
    <script>
        (function () {
            ...
            function initUploader(cfg) {
                ...
                // ---- FIXED: bubble events so Livewire sees the changes ----
                function syncHidden() {
                ...
                async function handleFiles(files) {
                    ...
                    for (const f of files) {
                        ...
                        try {
                            ...
                            // 2) Upload to Cloudinary (SIGNED) — send ONLY signed params + file
                            ...
                            const item = {
                                public_id: cloudJson.public_id,              // e.g. krbs/vehicle/tmp/{uid}/{tmpKey}/auto_name
                                secure_url: cloudJson.secure_url,
                                bytes: cloudJson.bytes || f.size,
                                resource_type: cloudJson.resource_type,      // image | raw | video
                                format: cloudJson.format,
                                original_filename: (cloudJson.original_filename || 'file') + (cloudJson.format ? '.' + cloudJson.format : '')
                            };

                            tempItems = [item];     // single file
                            syncHidden();           // <<<<< IMPORTANT
                            renderList();
                ...
                // expose sync helper for submit buttons
