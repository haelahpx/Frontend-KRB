...
class Guestbook extends Component
{
    ...
    /** ==== Normalizers (dipakai di updated* hooks) ==== */
    public function updatedDate($v): void               { $this->date            = $this->normalizeDate($v); }
    public function updatedEditDate($v): void           { $this->edit['date']    = $this->normalizeDate($v); }
    public function updatedJamIn($v): void              { $this->jam_in          = $this->normalizeTime($v); }
    public function updatedEditJamIn($v): void          { $this->edit['jam_in']  = $this->normalizeTime($v); }
    public function updatedEditJamOut($v): void         { $this->edit['jam_out'] = $this->normalizeTime($v, true); }
    ...
    /** Pastikan data milik company yang sama; tidak ikut yang sudah di-trashed kecuali denganTrashed() */
    ...
    /** SOFT DELETE: ini tidak menghapus permanen */
    public function delete(int $id): void
    {
        $row = $this->findOwnedOrFail($id);
        $row->delete(); // <-- soft delete, karena model pakai SoftDeletes
        $this->dispatch('toast', type: 'success', title: 'Dihapus', message: 'Guest dihapus.', duration: 3000);
        $this->dispatch('$refresh');
    }

    /** OPTIONAL: restore dari trash (pakai di UI kalau perlu) */
    ...
    /** OPTIONAL: hapus permanen dari trash (pakai di UI kalau perlu) */
    ...
    /** Tamu hari ini yang BELUM keluar (jam_out null). Soft-deleted otomatis tidak ikut. */
    ...
    /** Riwayat kunjungan (sudah keluar). Soft-deleted otomatis tidak ikut. */
