<div class="bg-gray-50"  wire:poll.800ms="tick" wire:poll.keep-alive.2s="tick">
            ...
            <section class="{{ $card }}">
                ...
                <div class="p-5 grid grid-cols-1 md:grid-cols-4 gap-5">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Catatan: status <b>Deleted</b> tidak tampil di tampilan
                            default. Pilih “Deleted” untuk melihat trash.</p>
                    </div>
                </div>
            </section>
            ...
            @if ($status !== 'deleted')
                <div class="space-y-6">
                                ...
                                <div class="p-3 rounded-lg bg-gray-50 border border-gray-200"
                                    ...
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="flex items-center gap-2 {{ $mono }}">
                                            ...
                                            <button class="{{ $btnRed }}"
                                                    wire:click="deleteTicket({{ $t->ticket_id }})"
                                                    wire:loading.attr="disabled"
                                                    wire:target="deleteTicket({{ $t->ticket_id }})">
                                                Delete
                                            </button>
                                        </div>
                                    </div>
                    ...
                    <section class="{{ $card }}">
                        ...
                        <div class="p-5 space-y-3">
                            @forelse ($closed as $t)
                                <div class="p-3 rounded-lg bg-gray-50 border border-gray-200 opacity-80"
                                    wire:key="cls-{{ $t->ticket_id }}">
                                    <div class="flex items-start gap-3">
                                        <div class="{{ $ico }}">{{ strtoupper(substr(($t->subject ?? 'T'), 0, 1)) }}</div>
                                        <div class="min-w-0 flex-1">
                                            <div class="flex items-center gap-2">
    ...
    @if($modalEdit)
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4" wire:key="modal-edit-ticket"
            ...
            <div
                ...
                <form class="p-5 space-y-4 overflow-y-auto flex-1" wire:submit.prevent="saveEdit">
                    <div>
                        ...
                        @enderror
                        <p class="mt-1 text-xs text-gray-500">Tetapkan agent untuk menandai tiket sebagai <b>In
                                Progress</b>.</p>
                    </div>
                    ...
                    <div
                        class="bg-gray-50 border-t border-gray-200 mt-4 -mx-5 px-5 py-4 flex items-center justify-end gap-2.5">
                        <button type="button"
                        ...
                        <button type="submit"
                            class="px-5 h-10 rounded-lg bg-gray-900 text-white text-sm font-medium hover:bg-black focus:outline-none focus:ring-2 focus:ring-gray-900/20 transition">
                            Save
                        </button>
                    </div>
                </form>
