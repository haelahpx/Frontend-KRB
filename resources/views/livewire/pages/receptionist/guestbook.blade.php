                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" wire:model.defer="phone_number" placeholder="08xxxxxxxxxx"
                            class="{{ $input }}">
                        @error('phone_number') <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" wire:model.defer="keperluan" placeholder="Tujuan kunjungan"
                            class="{{ $input }}">
                        @error('keperluan') <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
                <div>
                    <label class="{{ $label }}">👮 Nama Petugas Penjaga</label>
                    <input type="text" wire:model.defer="petugas_penjaga" placeholder="Nama petugas yang bertugas"
                        class="{{ $input }}">
                    @error('petugas_penjaga') <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                <div class="pt-2">
                    <button type="submit"
                        class="inline-flex items-center gap-2 px-4 h-10 rounded-lg bg-gray-900 text-white text-sm font-medium hover:bg-black focus:outline-none focus:ring-2 focus:ring-gray-900/20 shadow-sm">
                    ...
                    <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-200"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="leading-tight">
                                ...
                                <div class="text-[11px] text-gray-500">Masuk
                                    {{ \Carbon\Carbon::parse($r->jam_in)->format('H:i') }}</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <button wire:click="openEdit({{ $r->guestbook_id }})" wire:loading.attr="disabled"
                                wire:target="openEdit({{ $r->guestbook_id }})" class="{{ $btnBlk }}">
                            ...
                            <button wire:click="setJamKeluarNow({{ $r->guestbook_id }})" wire:loading.attr="disabled"
                                wire:target="setJamKeluarNow({{ $r->guestbook_id }})" class="{{ $btnGrn }}">
                                <span wire:loading.remove wire:target="setJamKeluarNow({{ $r->guestbook_id }})">Keluar
                                    sekarang</span>
            ...
            <div class="px-6 py-5 bg-gray-50 border-b border-gray-200">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-center">
                    ...
                    <div class="relative flex-1">
                        <input type="text" wire:model.live="q" placeholder="Cari nama, no hp, instansi, atau petugas..."
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                            <div class="flex items-start gap-3 flex-1">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center gap-2 mb-1.5">
                                        ...
                                        @if ($e->phone_number)
                                            <span
                                                class="text-[11px] text-gray-600 bg-gray-100 px-2 py-0.5 rounded-md">{{ $e->phone_number }}</span>
                                        @endif
                                    </div>
                            ...
                            <div class="text-right shrink-0 space-y-2">
                                ...
                                <div class="flex flex-wrap gap-2 justify-end pt-1.5">
                                    <button wire:click="openEdit({{ $e->guestbook_id }})" wire:loading.attr="disabled"
                                        wire:target="openEdit({{ $e->guestbook_id }})" class="{{ $btnBlk }}">
                                    ...
                                    <button wire:click="delete({{ $e->guestbook_id }})"
                                        onclick="return confirm('Hapus entri ini?')" wire:loading.attr="disabled"
                ...
                <div class="p-5 space-y-4 overflow-y-auto flex-1">
                    ...
                    <div class="grid grid-cols-2 gap-3.5">
                        <div class="space-y-1.5">
                            ...
                            @error('edit.jam_in') <p class="text-[11px] text-red-600 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="space-y-1.5">
                            <label class="{{ $label }}">Jam Keluar</label>
                            <input type="time" wire:model="edit.jam_out" class="{{ $editIn }}">
                            @error('edit.jam_out') <p class="text-[11px] text-red-600 font-medium">{{ $message }}</p>
                            @enderror
                            <div class="bg-gray-50 rounded-md p-3 border border-gray-200">
                                <p class="text-[11px] text-gray-600 leading-relaxed">
                                    💡 <span class="font-medium">Tips:</span> Klik <span
                                        class="font-semibold text-gray-900">Keluar sekarang</span> di "Kunjungan Terbaru"
                                    untuk menggunakan waktu real-time.
                                </p>
                            </div>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-3.5">
                        <div class="space-y-1.5">
                            ...
                            <input type="text" wire:model="edit.name" placeholder="Masukkan nama lengkap"
                                class="{{ $editIn }}">
                            @error('edit.name') <p class="text-[11px] text-red-600 font-medium">{{ $message }}</p> @enderror
                        </div>
                        <div class="space-y-1.5">
                            <label class="{{ $label }}">No HP</label>
                            <input type="text" wire:model="edit.phone_number" placeholder="08xxxxxxxxxx"
                                class="{{ $editIn }}">
                        </div>
                    </div>
                    <div class="space-y-3.5">
                        <div class="space-y-1.5">
                            ...
                            <input type="text" wire:model="edit.instansi" placeholder="Nama perusahaan/instansi"
                                class="{{ $editIn }}">
                        </div>
                        <div class="space-y-1.5">
                            <label class="{{ $label }}">Keperluan</label>
                            <input type="text" wire:model="edit.keperluan" placeholder="Tujuan kunjungan"
                                class="{{ $editIn }}">
                        </div>
                        <div class="space-y-1.5">
                            <label class="{{ $label }}">Petugas Penjaga</label>
                            <input type="text" wire:model="edit.petugas_penjaga" placeholder="Nama petugas yang bertugas"
                                class="{{ $editIn }}">
                            @error('edit.petugas_penjaga') <p class="text-[11px] text-red-600 font-medium">{{ $message }}
                            </p> @enderror
                        </div>
                    </div>
                </div>
