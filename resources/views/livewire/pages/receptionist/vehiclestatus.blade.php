        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-gray-900 to-black text-white shadow-2xl">
            ...
            <div class="relative z-10 p-6 sm:p-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-white/80">Pantau peminjaman kendaraan yang masih Pending atau sedang
                                In Use.</p>
                        </div>
                    </div>
                ...
                @if(session()->has('success'))
                    <div
                        class="mt-4 flex items-start gap-2 text-sm text-emerald-100 bg-emerald-900/30 border border-emerald-500/30 rounded-xl px-4 py-3 backdrop-blur-sm">
                ...
                <div class="px-4 sm:px-6 pt-4 pb-3 border-b border-gray-200 space-y-3">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        ...
                        <div class="inline-flex items-center bg-gray-100 rounded-full p-1 text-xs font-medium">
                            <button type="button" wire:click="$set('statusTab','pending')" class="px-3 py-1 rounded-full transition
                                        {{ $statusTab === 'pending'
    ? 'bg-gray-900 text-white shadow-sm'
    : 'text-gray-700 hover:bg-gray-200' }}">
                                Pending
                                <button type="button" wire:click="$set('statusTab','approved')" class="px-3 py-1 rounded-full transition
                                            {{ $statusTab === 'approved'
                            ? 'bg-gray-900 text-white shadow-sm'
                            : 'text-gray-700 hover:bg-gray-200' }}">
                                    Approved
                            ...
                            <button type="button" wire:click="$set('statusTab','on_progress')" class="px-3 py-1 rounded-full transition
                                        {{ $statusTab === 'on_progress'
    ? 'bg-gray-900 text-white shadow-sm'
    : 'text-gray-700 hover:bg-gray-200' }}">
                                On Progress
                            </button>
                            <button type="button" wire:click="$set('statusTab','returned')" class="px-3 py-1 rounded-full transition
                                        {{ $statusTab === 'returned'
    ? 'bg-gray-900 text-white shadow-sm'
    : 'text-gray-700 hover:bg-gray-200' }}">
                                Returned
                        ...
                        @if(!is_null($vehicleFilter))
                            ...
                            <span
                                ...
                                <button type="button" class="ml-1 hover:text-gray-200"
                                    wire:click="$set('vehicleFilter', null)">×</button>
                            </span>
                        @else
                            <span
                            ...
                            <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                                <div class="flex items-start gap-3 flex-1 min-w-0">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-center gap-2 mb-1.5">
                                            ...
                                            <span
                                                class="text-[11px] px-2 py-0.5 rounded-full {{ $statusStyle['bg'] }} {{ $statusStyle['text'] }}">
                                                {{ $statusStyle['label'] }}
                                            </span>
                                            <span
                                                class="text-[11px] px-2 py-0.5 rounded-full border border-gray-300 text-gray-700 bg-gray-50">
                                                #{{ $b->vehiclebooking_id }}
                                            </span>
                                        </div>
                                        ...
                                        <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-600">
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-gray-50 border border-gray-200">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="text-right shrink-0 space-y-2">
                                    <div class="flex flex-wrap gap-2 justify-end pt-1.5">
                                        @if($b->status === 'pending')
                                            <button type="button" wire:click="reject({{ $b->vehiclebooking_id }})"
                                            ...
                                            <button type="button" wire:click="approve({{ $b->vehiclebooking_id }})"
                                        ...
                                        @elseif($b->status === 'on_progress')
                                            <button type="button" wire:click="markReturned({{ $b->vehiclebooking_id }})"
                                                wire:target="markReturned"
                                        ...
                                        @elseif(in_array($b->status, ['returned']))
                                            <button type="button" wire:click="markDone({{ $b->vehiclebooking_id }})"
                                                @if($afterC === 0) disabled @endif wire:loading.attr="disabled"
                                                wire:target="markDone" class="px-3 py-2 text-xs font-medium rounded-lg
                                                                        @if($afterC === 0)
            ...
            <aside class="hidden md:flex md:flex-col md:col-span-1 gap-4">
                ...
                <section class="{{ $card }}">
                    <div class="px-4 py-4 border-b border-gray-200">
                        ...
                        <p class="text-xs text-gray-500 mt-1">Klik salah satu kendaraan untuk mem-filter daftar booking.
                        </p>
                    </div>
                    ...
                    <div class="px-4 py-3 max-h-64 overflow-y-auto">
                        ...
                        <button type="button" wire:click="$set('vehicleFilter', null)"
                            ...
                            <span class="flex items-center gap-2">
                                <span
                                    class="inline-flex items-center justify-center w-6 h-6 rounded-md border border-gray-300 text-[11px]">
                                    All
                                </span>
