<div class="min-h-screen bg-gray-50" wire:poll.1000ms>
    ...
    <div class="px-4 sm:px-6 py-6 space-y-8">
        ...
        {{-- FILTER BAR (match BookingsApproval) --}}
        <section class="{{ $card }}">
            <div class="px-6 py-5 bg-gray-50 border-b border-gray-200">
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            <input type="text" class="{{ $input }} pl-9" placeholder="Cari nama, no HP, instansi, petugas, keperluan…" wire:model.live="q">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="date" class="{{ $input }} pl-9" wire:model.live="filter_date">
                            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="{{ $card }}">
                ...
                <div class="divide-y divide-gray-200">
                    @forelse ($latest as $r)
                        ...
                        <div class="px-6 py-5 hover:bg-gray-50 transition-colors" wire:key="latest-{{ $r->guestbook_id }}">
                            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                {{-- LEFT: DATA --}}
                                <div class="flex items-start gap-3 flex-1 min-w-0">
                                    <div class="{{ $icoAvatar }}">{{ strtoupper(substr($r->name ?? '—', 0, 1)) }}</div>
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center gap-2 mb-1.5">
                                            <h4 class="font-semibold text-gray-900 text-base truncate">{{ $r->name }}</h4>
                                            @if ($r->phone_number)
                                                <span class="text-[11px] text-gray-600 bg-gray-100 px-2 py-0.5 rounded-md">{{ $r->phone_number }}</span>
                                            @endif
                                        </div>
                                        ...
                                        <div class="flex flex-wrap items-center gap-4 text-[13px] text-gray-600">
                                            ...
                                            {{-- ✅ Clock-in (green) and clock-out (red) icons --}}
                                            <span class="flex items-center gap-1.5">
                                ...
                                <div class="text-right shrink-0 space-y-2">
                                    @if(!empty($r->created_at))
                                        <div class="text-[11px] text-gray-500">{{ \Carbon\Carbon::parse($r->created_at)->format('d M Y H:i') }}</div>
                                    @endif
            ...
            <div class="{{ $card }}">
                ...
                <div class="divide-y divide-gray-200">
                    @forelse ($entries as $e)
                        @php $rowNo = ($entries->firstItem() ?? 1) + $loop->index; @endphp
                        <div class="px-6 py-5 hover:bg-gray-50 transition-colors" wire:key="entry-{{ $e->guestbook_id }}-{{ $e->deleted_at ? 'trash' : 'ok' }}">
                            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                {{-- LEFT: DATA --}}
                                <div class="flex items-start gap-3 flex-1 min-w-0">
                                    <div class="{{ $icoAvatar }}">{{ strtoupper(substr($e->name ?? '—', 0, 1)) }}</div>
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-center gap-2 mb-1.5">
                                            <h4 class="font-semibold text-gray-900 text-base truncate">{{ $e->name }}</h4>
                                            @if ($e->phone_number)
                                ...
                                {{-- RIGHT: ACTIONS + NUMBER --}}
                                <div class="text-right shrink-0 space-y-2">
                                    <div class="text-[11px] text-gray-500">{{ \Carbon\Carbon::parse($e->created_at)->format('d M Y H:i') }}</div>
                                    ...
                                    <div class="flex flex-wrap gap-2 justify-end pt-1.5">
                                        ...
                                        @else
                                            ...
                                            <button wire:click="restore({{ $e->guestbook_id }})"
                                                ...
                                                <span wire:loading.remove wire:target="restore({{ $e->guestbook_id }})">Restore</span>
                                                <span wire:loading wire:target="restore({{ $e->guestbook_id }})">Memproses…</span>
                                            </button>
                                            ...
                                            <button wire:click="destroyForever({{ $e->guestbook_id }})"
                                                ...
                                                <span wire:loading.remove wire:target="destroyForever({{ $e->guestbook_id }})">Hapus Permanen</span>
                                                <span wire:loading wire:target="destroyForever({{ $e->guestbook_id }})">Menghapus…</span>
                                            </button>
                                        @endif
                                    </div>
