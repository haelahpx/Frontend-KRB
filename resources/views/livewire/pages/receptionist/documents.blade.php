            ...
            <form wire:submit.prevent="save" class="p-6 space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        @error('document_name') <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="{{ $label }}">🏷️ Type</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="{{ $label }}">🗄️ Penyimpanan</label>
                    ...
                    <div>
                        ...
                        @error('pengambilan_date') <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p> @enderror
                        @error('pengambilan_time') <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="{{ $label }}">🏢 Department (opsional)</label>
                        <input type="number" wire:model.defer="department_id" class="{{ $input }}" placeholder="ID Department">
                        @error('department_id') <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="{{ $label }}">⚙️ Status</label>
                ...
                <div class="pt-2 flex items-center gap-3">
                    <button type="submit" wire:loading.attr="disabled" wire:target="save" class="inline-flex items-center gap-2 px-4 h-10 rounded-lg bg-gray-900 text-white text-sm font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-900/20 transition active:scale-95 hover:bg-black disabled:opacity-60 relative overflow-hidden">
        ...
        <div class="{{ $card }}">
            ...
            <div class="p-5 space-y-3">
                @forelse ($pendingList as $r)
                    <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-200" wire:key="pending-{{ $r->document_id }}">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="leading-tight">
                                ...
                                <div class="text-[11px] text-gray-500">
                                    Pengambilan {{ optional($r->pengambilan)->format('H:i') ?? '—' }} • Pengirim {{ $r->nama_pengirim ?? '—' }}
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <button wire:click="openEdit({{ $r->document_id }})" wire:loading.attr="disabled" wire:target="openEdit({{ $r->document_id }})" class="{{ $btnBlk }}">
                            ...
                            <button wire:click="setSudahDikirim({{ $r->document_id }})" wire:loading.attr="disabled" class="{{ $btnGrn }}">
                                <span wire:loading.remove>Sudah dikirim</span>
        ...
        <div class="{{ $card }}">
            ...
            <div class="p-5 space-y-3">
                @forelse ($takenList as $r)
                    <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-200" wire:key="taken-{{ $r->document_id }}">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 bg-gray-900 rounded-lg flex items-center justify-center text-white text-xs font-semibold">
                                {{ strtoupper(substr($r->document_name, 0, 1)) }}
                            </div>
            ...
            <div class="px-6 py-5 bg-gray-50 border-b border-gray-200">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-center">
                    ...
                    <div class="relative flex-1">
                        <input type="text" wire:model.live="q" placeholder="Cari nama dokumen / pengirim / penerima / type / penyimpanan / status..." class="{{ $input }} pl-9 w-full">
                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                            ...
                            <div class="text-right shrink-0 space-y-2">
                                ...
                                <div class="flex flex-wrap gap-2 justify-end pt-1.5">
                                    <button wire:click="openEdit({{ $e->document_id }})" wire:loading.attr="disabled" wire:target="openEdit({{ $e->document_id }})" class="{{ $btnBlk }}">
                                    ...
                                    <button wire:click="delete({{ $e->document_id }})" onclick="return confirm('Hapus dokumen ini?')" wire:loading.attr="disabled" wire:target="delete({{ $e->document_id }})" class="px-3 py-2 text-xs font-medium rounded-lg bg-rose-600 text-white hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-rose-600/20 disabled:opacity-60 transition">
            ...
            <div class="relative w-full max-w-lg bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden transform transition-all duration-300 scale-100 max-h-[90vh] flex flex-col">
                <div class="bg-gradient-to-r from-gray-900 to-black p-5 text-white relative overflow-hidden">
                    ...
                    <div class="relative z-10 flex items-center justify-between">
                        ...
                        <button class="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 flex items-center justify-center transition-all duration-200" wire:click="closeEdit">
                            <svg class="w-4.5 h-4.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        </button>
                    </div>
                </div>
                ...
                <div class="p-5 space-y-4 overflow-y-auto flex-1">
                    <div class="space-y-1.5">
                        ...
                        @error('edit.document_name') <p class="text-[11px] text-red-600 font-medium">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        @error('edit.pengambilan_date') <p class="text-[11px] text-red-600 font-medium">{{ $message }}</p>@enderror
                        @error('edit.pengambilan_time') <p class="text-[11px] text-red-600 font-medium">{{ $message }}</p>@enderror
                        <div class="bg-gray-50 rounded-md p-3 border border-gray-200 mt-1.5">
                            <p class="text-[11px] text-gray-600 leading-relaxed">
                                💡 <span class="font-medium">Tips:</span> Klik <span class="font-semibold text-gray-900">Sudah dikirim</span> di daftar Pending/Taken untuk pakai waktu real-time pengiriman.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3.5">
                        <div class="space-y-1.5">
                            ...
                            @error('edit.pengiriman') <p class="text-[11px] text-red-600 font-medium">{{ $message }}</p>@enderror
                        </div>
                        <div class="space-y-1.5">
                            <label class="{{ $label }}">Nama Pengirim</label>
                    ...
                    <div class="grid grid-cols-2 gap-3.5">
                        ...
                        <div class="space-y-1.5">
                            <label class="{{ $label }}">Department (opsional)</label>
                            <input type="number" wire:model="edit.department_id" class="{{ $editIn }}" placeholder="ID Department">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3.5">
                        <div class="space-y-1.5">
                            ...
                            @error('edit.status') <p class="text-[11px] text-red-600 font-medium">{{ $message }}</p>@enderror
                            <p class="text-[11px] text-gray-500 mt-1">
