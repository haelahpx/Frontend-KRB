    ...
    <main class="px-4 sm:px-6 py-6 space-y-8">
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-gray-900 to-black text-white shadow-2xl">
            ...
            <div class="relative z-10 p-6 sm:p-8">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <h2 class="text-lg sm:text-xl font-semibold">Manage Requirements</h2>
                        <p class="text-sm text-white/80">Scope: <span class="font-semibold">Global</span></p>
                    </div>
        ...
        <section class="{{ $card }}">
            <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between gap-3">
                ...
                <div class="w-72">
                    <input type="text" wire:model.live="req_search" class="{{ $input }} h-10"
            ...
            <form class="p-5" wire:submit.prevent="reqStore">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" wire:model.defer="req_name" class="{{ $input }}"
                            placeholder="e.g. Projector, Whiteboard">
                        @error('req_name') <p class="mt-1 text-xs text-rose-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
                <div class="pt-5">
            ...
            <div class="divide-y divide-gray-200">
                @forelse ($requirements as $req)
                    @php
                        $rowNo = (($requirements->firstItem() ?? 1) + $loop->index);
                    @endphp
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2 mt-2">
                                        <span class="{{ $chip }}"><span class="text-gray-500">Created:</span><span
                                                class="font-medium text-gray-700">{{ $req->created_at?->format('d M Y, H:i') }}</span></span>
                                        <span class="{{ $chip }}"><span class="text-gray-500">Updated:</span><span
                                                class="font-medium text-gray-700">{{ $req->updated_at?->format('d M Y, H:i') }}</span></span>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="flex flex-wrap gap-2 justify-end pt-1">
                                    <button wire:click="reqOpenEdit({{ $req->requirement_id }})" class="{{ $btnBlk }}"
                                        ...
                                        <span wire:loading.remove
                                            wire:target="reqOpenEdit({{ $req->requirement_id }})">Edit</span>
                                        <span wire:loading
                                            wire:target="reqOpenEdit({{ $req->requirement_id }})">Loading…</span>
                                    </button>

                                    <button wire:click="reqDelete({{ $req->requirement_id }})"
                                        onclick="return confirm('Hapus requirement ini?')" class="{{ $btnRed }}"
                                        wire:loading.attr="disabled" wire:target="reqDelete({{ $req->requirement_id }})"
                                        wire:key="btn-del-req-{{ $req->requirement_id }}">
                                        <span wire:loading.remove
                                            wire:target="reqDelete({{ $req->requirement_id }})">Delete</span>
                                        <span wire:loading
                                            wire:target="reqDelete({{ $req->requirement_id }})">Deleting…</span>
                                    </button>
                                </div>
                            </div>
                        </div>
        ...
        @if($reqModal)
            <div class="fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true"
                ...
                <button type="button" class="absolute inset-0 bg-black/50" aria-label="Close overlay"
                    wire:click="reqCloseEdit"></button>
                <div class="relative w-full max-w-xl mx-4 {{ $card }} focus:outline-none" tabindex="-1">
                    <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
                        ...
                        <button class="text-gray-500 hover:text-gray-700" type="button" wire:click="reqCloseEdit"
                            aria-label="Close">
                    ...
                    <form class="p-5" wire:submit.prevent="reqUpdate">
                        <div class="space-y-5">
                            <div>
                                ...
                                @error('req_edit_name') <p class="mt-1 text-xs text-rose-600 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        <div class="mt-6 flex items-center justify-end gap-3 border-t border-gray-200 pt-4">
                            ...
                            <button type="submit" wire:loading.attr="disabled" wire:target="reqUpdate"
                                class="{{ $btnBlk }}">
