                                ...
                                <div class="flex items-start gap-3 flex-1 min-w-0">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-600">
                                            ...
                                            @if($includeDeleted)
                                                <span class="text-[11px] text-gray-500">(* termasuk yang terhapus)</span>
                                            @endif
                                        </div>
                                        ...
                                        <div class="mt-3 space-y-3">
                                            {{-- Before --}}
                                            <div>
                                                <div class="text-[11px] font-semibold text-gray-700 mb-1">Before</div>
                                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                                    @forelse($before as $p)
                                                        <div class="relative group border rounded-xl overflow-hidden bg-gray-50">
                                                            <img src="{{ photoUrl($p->photo_path) }}" alt="before" class="w-full h-28 object-cover">
                                                            @if($p->deleted_at)
                                                                <span class="absolute top-2 left-2 text-[10px] bg-rose-600 text-white px-1.5 py-0.5 rounded">Deleted</span>
                                                            @endif
                                                            <div class="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/60 to-black/0 opacity-0 group-hover:opacity-100 transition flex gap-1 justify-end">
                                                                @if(!$p->deleted_at)
                                                                    <button type="button" class="text-[11px] px-2 py-1 rounded bg-white/90"
                                                                            wire:click="deletePhoto({{ $p->id }})">Soft Delete</button>
                                                                @else
                                                                    <button type="button" class="text-[11px] px-2 py-1 rounded bg-white/90"
                                                                            wire:click="restorePhoto({{ $p->id }})">Restore</button>
                                                                    <button type="button" class="text-[11px] px-2 py-1 rounded bg-rose-600 text-white"
                                                                            wire:click="forceDeletePhoto({{ $p->id }})">Force Delete</button>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    @empty
                                            ...
                                            {{-- After --}}
                                            <div>
                                                <div class="text-[11px] font-semibold text-gray-700 mb-1">After</div>
                                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                                    @forelse($after as $p)
                                                        <div class="relative group border rounded-xl overflow-hidden bg-gray-50">
                                                            <img src="{{ photoUrl($p->photo_path) }}" alt="after" class="w-full h-28 object-cover">
                                                            @if($p->deleted_at)
                                                                <span class="absolute top-2 left-2 text-[10px] bg-rose-600 text-white px-1.5 py-0.5 rounded">Deleted</span>
                                                            @endif
                                                            <div class="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/60 to-black/0 opacity-0 group-hover:opacity-100 transition flex gap-1 justify-end">
