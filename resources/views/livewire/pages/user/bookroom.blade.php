        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <form wire:submit="submitBooking" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select wire:model="room_id" class="w-full px-3 py-2 border text-gray-900 border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent">
                                    <option value="">Select room</option>
                                    @foreach($rooms as $room)
                                        <option value="{{ $room['id'] }}" {{ !$room['available'] ? 'disabled' : '' }}>
                                            {{ $room['name'] }} ({{ $room['capacity'] }} people) {{ !$room['available'] ? '- Occupied' : '' }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <label class="flex items-center space-x-2">
                                    <input type="checkbox" wire:model="requirements" value="projector" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900">
                                    <span class="text-sm text-gray-900">Projector</span>
                                </label>
                                <label class="flex items-center space-x-2">
                                    <input type="checkbox" wire:model="requirements" value="whiteboard" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900">
                                    <span class="text-sm text-gray-900">Whiteboard</span>
                                </label>
                                <label class="flex items-center space-x-2">
                                    <input type="checkbox" wire:model="requirements" value="video_conference" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900">
                                    <span class="text-sm text-gray-900">Video Conference</span>
                                </label>
                                <label class="flex items-center space-x-2">
                                    <input type="checkbox" wire:model="requirements" value="catering" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900">
                        ...
                        <div>
                            ...
                            <textarea wire:model="special_notes" placeholder="Any additional information or special requirements..." rows="4"
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-lg shadow-sm border-2 border-black p-6">
                    ...
                    <div class="space-y-3">
                        @foreach($rooms as $room)
                        <div class="flex items-center justify-between p-3 {{ $room['available'] ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200' }} border rounded-md">
                            <div>
                                <div class="flex items-center space-x-2">
                                    <div class="w-2 h-2 {{ $room['available'] ? 'bg-green-500' : 'bg-red-500' }} rounded-full"></div>
            ...
            <div class="bg-gray-50 border-b-2 border-black p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Week of {{ $currentWeek->format('M j') }} - {{ $currentWeek->copy()->addDays(6)->format('M j, Y') }}</p>
                    </div>
                    <div class="flex items-center space-x-4">
                        <button wire:click="previousWeek" class="p-2 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">
                            <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </button>
                        <button wire:click="nextWeek" class="p-2 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">
                            <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="border-b border-gray-200">
                        <div class="grid grid-cols-8 bg-gray-50 border-b border-gray-100">
                            <div class="col-span-8 p-3 border-r border-gray-200">
                                <div class="flex items-center space-x-2">
                                    <div class="w-3 h-3 {{ $room['available'] ? 'bg-green-500' : 'bg-red-500' }} rounded-full"></div>
                                    <span class="font-medium text-gray-900">{{ $room['name'] }}</span>
